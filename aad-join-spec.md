...
<table class="protocol-table"><thead>
 ...
 <tr>
  ...
  <td>
  <p>
    <span><a href="https://github.com/himmelblau-idm/aad-join-spec/releases/download/0.03/aad-join-spec.pdf" data-linktype="external">PDF</a></span>
  | <span><a href="https://github.com/himmelblau-idm/aad-join-spec/releases/download/0.03/aad-join-spec.html" data-linktype="external">HTML</a></span>
  </p>
  </td>
 </tr>
...
- __KeySecurity__: A property that identifies how the private key associated
with the certificate request is protected. A software-backed key MUST specify
`Software`. Optional.

- __KeyType__: A property that identifies the asymmetric key algorithm used by
the certificate request. For RSA certificate requests, this value MUST be
`RSA`. Optional.

__DeviceKeys__: An array of public keys generated by the client. This property
is an alternate representation of the TransportKey used by API version 3.0. If
specified, the array MUST contain a key whose Usage is `STK`, and the client
MUST retain the corresponding private key.

- __Data__: A base64-encoded UTF-8
[JSON Web Key (JWK)](https://datatracker.ietf.org/doc/html/rfc7517) containing
the public portion of an asymmetric key. For an RSA key, the decoded JSON object
MUST contain the `kty` member set to `RSA`, the `e` member set to the base64url
encoded public exponent, and the `n` member set to the base64url encoded
modulus. Required.

- __Encoding__: A property that identifies the encoding used for Data. This
value MUST be `Base64`. Required.

- __Usage__: A property that identifies the purpose of the key. The value `STK`
identifies the secure transport key. Required.
...
- __ObjectId__: The object id of the identity that authenticated to the Web service,
or the registered owner of the device.
